...
public class ApMon {
	...
	public static final int MAX_DGRAM_SIZE = 8192;/**< Maximum UDP datagram size. */
	...
	public static final int DEFAULT_PORT = 8884; /**< The default port on which MonALISA listens */
	...
	/**
	 * The number of time intervals at which ApMon sends general system 
	 * monitoring information (considering the time intervals at which ApMon 
	 * sends system monitoring information).
	 */
	...
	String clusterName;	/**< The name of the monitored cluster. */
	String nodeName;	/**< The name of the monitored node. */ 
	
	Vector destAddresses;	/**< The IP addresses where the results will be sent.*/
	Vector destPorts; 	/**< The ports where the destination hosts listen. */
	Vector destPasswds;	/**< The Passwdords used for the destination hosts. */
	
	byte []buf;	/**< The buffer which holds the message data (encoded in XDR). */
	int dgramSize;	/**< The size of the data inside the datagram */
	
	/** Hashtable which holds theinitialization resources (Files, URLs) 
	...
	/** The background thread which performs operations like checking the configuration
	 * file/URLs for changes and sending datagrams with monitoring information. */
	 ...
	 * read from proc/, ApMon will not attempt to include that value in the next datagrams. */
	...
	/** If this flag is true, packets with general system information taken 
	 * from /proc are periodically sent to MonALISA */
	...
	/** Hashtables that associate the names of the parameters included in
	 * the monitoring datagrams and flags that indicate whether they are active or not. */ 
	long sysMonitorParams, jobMonitorParams, genMonitorParams;
	
	/** The time when the last datagram with job monitoring information was sent (in
	 ...
	 * (the object is being constructed now). Else, only some structures will be reinitialized.
	 * @throws ApMonException, SocketException, IOException
	 */
	 ...
	 * @param nDestinations The number of destination hosts where the results will be sent.
	...
	throws ApMonException, SocketException, IOException {
		 ...
		 * buffers (because we don't want to keep the monitor while making DNS requests)*/
		...
		/** start job/system monitoring according to the settings previously 
	...
	/**
	 * Sends several parameters and their values to the MonALISA module. 
	 * @param clusterName The name of the cluster that is monitored.
	 * @param nodeName The name of the node from the cluster from which the value was taken.
	 * @param paramNames Vector with the names of the parameters.
	 * @param valueTypes Vector with the value types of the parameters. The
	 ...
	 * @param paramName The name of the parameter.
	 * @param valueType The value type of the parameter. Can be one of the 
	 * constants XDR_INT32 (integer),XDR_REAL64 (double), XDR_STRING.
	 * @param paramValue The value of the parameter.
	 * @throws ApMonException, UnknownHostException, SocketException
	 * @param timestamp The user's timestamp
	 */
	 ...
	 * @param confRecheck If it is true, the configuration rechecking is enabled.
	 * @param interval The time interval at which the verifications are done. The interval
	 * will be automatically increased if ApMon cannot connect to the configuration URLs.
	 */
	...
	/**
	 * Returns the requested value of the time interval (in seconds) between 
	 ...
	 * recheck operations for the configuration file/URLs.
	 */
	 ...
	 * operations for the configuration file/URLs. 
	 ...
	 * @param sysMonitoring If it is true, the job monitoring is enabled.
	 * @param interval The time interval at which the job monitoring datagrams
	 * are sent.
	 */
	 ...
	 * @param interval The time interval at which the system monitoring datagrams
	 * are sent.
	 */
	 ...
	 * @param interval The number of time intervals at which the general system 
	 * monitoring datagrams are sent (a "time interval" is the time interval between
	 * two subsequent system monitoring datagrams).
	 */
	...
	void setBackgroundThread(boolean val) {
		...
		if (stoppedThread) {
			try {
				//debugging stuff...
				/*
				System.out.println("### active count: " + bkThread.activeCount());
				Thread[] tarray = new Thread[500];
				bkThread.enumerate(tarray);
				for (int tz = 0; tz < tarray.length; tz++) {
					if (tarray[tz] != null)
						System.out.println("### " + tarray[tz]);
				}
				*/
				bkThread.join();
			} catch (Exception e) {}
    ...
    /**
     * Sets the ApMon loglevel. The possible values are: "FATAL", "WARNING", 
     * "INFO", "FINE", "DEBUG".
     */ 
	...
	/**
	 * Must be called when the ApMon object is no longer in use. Closes the UDP socket 
	 * used for sending the parameters and sends a last job monitoring datagram to
	 * register the time when the application was finished. 
	 */
	public void stopIt() {
	...
	/***
	 * Parses a "xApMon" line from the configuration file and changes the ApMon settings 
	...
	protected void parseXApMonLine(String line) {
		...
		/***
		 * mutexBack protects the variables which hold settings for the backgrund thread
		 * (for sending monitoring information and for rechecking the configuration file/URLS) 
		 */
		synchronized(mutexBack) {
            found = false;
            Long val = null;
	 ...
	 * @param valueTypes Vector of Integers which represent the value types of the parameters.
	 * @param paramValues Vector with the values of the parameters.
	 */
